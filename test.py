...
def test(model, a2c, config, early_break=True, batch_size=None, verbose=False):
    ...
    d = torchvision.datasets.MNIST(root="./",
                                   train=False,
                                   download=True,
    ...
    for i, (ori_image, image, mask) in enumerate(test_loader):
        ...
        for t in range(config.episode_len):
            ...
            image_input = Variable(image.cuda(), volatile=True)
            pi_out, v_out = model(image_input, flag_a2c=True)
            ...
            # actions = actions.astype(torch.uint8)
